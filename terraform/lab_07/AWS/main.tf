...
resource "aws_subnet" "public_a" {
  ...
  tags = merge(local.tags, {
    Name = "${local.name_prefix}public-subnet-public-subnet-us-east-1a"
  })
}
...
resource "aws_subnet" "public_b" {
  ...
  tags = merge(local.tags, {
    Name = "${local.name_prefix}public-subnet-public-subnet-us-east-1b"
  })
}
...
resource "aws_subnet" "private_a" {
  ...
  tags = merge(local.tags, {
    Name        = "${local.name_prefix}private-subnet-${data.aws_region.current.region}"
    Environment = local.tags.Environment
    Project     = local.tags.Project
    Owner       = local.tags.Owner
    CostCenter  = local.tags.CostCenter
